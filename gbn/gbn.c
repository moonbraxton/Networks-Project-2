...
// defines attributes for sending end (A in this case)
struct Sender {
    int base;
    int nextseq;
    int window_size;
...
// defines attributes for receiving end
struct Receiver {
    int expect_seq;
...
// calculate and return the checksum, either for corruption checking or message sending (B in this case)
...
// responsible for sending all messages in the 'window'
void send_window(void) {
    while (A.nextseq < A.buffer_next && A.nextseq < A.base + A.window_size) {
        ...
        printf("   send_window: Sending packet (seq=%d): %s\n", packet->seqnum, packet->payload);
        tolayer3(0, *packet);
...
/*
    called whenever a packet sent from the B-side (i.e., as a result of a tolayer3() being done by a B-side procedure) 
    arrives at the A-side. packet is the
    (possibly corrupted) packet sent from the B-side
*/
void A_input(struct pkt packet) {
    //if the packet checksum is incorrect, drop the packet
    if (packet.checksum != get_checksum(&packet)) {
        printf("   A_input: Found corrupted packet. Dropping.\n");
        return;
    }
    ...
    printf("   A_input: Received ACK (ack=%d)\n", packet.acknum);
    A.base = packet.acknum + 1;

    //if the next packet is ready to send, send it
    if (A.base == A.nextseq) {
        stoptimer(0);
    ...
    //if the next packet is not ready to be sent, wait until it is
    else {
        starttimer(0, A.estimated_rtt);
...
/*
    ...
    being done by a A-side procedure) arrives at the B-side. packet is the
    (possibly corrupted) packet sent from the A-side.

    Note that with simplex transfer from a-to-B, there is no B_output()

*/
void B_input(struct pkt packet) {
    //if the packet checksum is incorrect, send a NAK to A
    if (packet.checksum != get_checksum(&packet)) {
        printf("   B_input: Received corrupted packet. Sending NAK (ack=%d)\n", B.packet_to_send.acknum);
        tolayer3(1, B.packet_to_send);
        return;
    }

    //if the packet seqnum is incorrect, send a NAK to A
    if (packet.seqnum != B.expect_seq) {
        printf("   B_input: Received unexpected seq. Sending NAK (ack=%d)\n", B.packet_to_send.acknum);
        tolayer3(1, B.packet_to_send);
        return;
    }

    //if everything in packet is correct, send appropriate ACK
    printf("   B_input: recv packet (seq=%d): %s\n", packet.seqnum, packet.payload);
    tolayer5(1, packet.payload);

    printf("   B_input: Sending ACK (ack=%d)\n", B.expect_seq);
...
/*
    called whenever the upper layer at the sending side (A) has a message to send. It is the job of your
    protocol to insure that the data in such a message is delivered in-order, and correctly, to the receiving side upper
    layer.
*/
void A_output(struct msg message) {
    //If the buffer is full, drop the message
    if (A.buffer_next - A.base >= BUFSIZE) {
        printf("   A_output: The buffer is full. Droping the message: %s\n", message.data);
        return;
    }
...
void B_output(struct msg message) {
    printf("   B_output: Uni-direction only. Ignoring\n");
}

/*
    called when A's timer expires (thus generating a timer interrupt). this routine is used 
    to control the retransmission of packets. See starttimer() and stoptimer()
...
void A_timerinterrupt(void) {
    for (int i = A.base; i < A.nextseq; ++i) {
        ...
        printf("   A_timerinterrupt: Resending packet (seq=%d): %s\n", packet->seqnum, packet->payload);
        tolayer3(0, *packet);
    }
    starttimer(0, A.estimated_rtt);
...
void B_timerinterrupt(void) {
    printf("   B_timerinterrupt: B doesn't have a timer. Ignoring.\n");
}
